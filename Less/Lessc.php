...
class lessc_parser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('Less_Lessc', 'preg_quote'),
					array_keys(self::$precedence))).')';

			$commentSingle = Less_Lessc::preg_quote(self::$commentSingle);
			$commentMultiLeft = Less_Lessc::preg_quote(self::$commentMultiLeft);
			$commentMultiRight = Less_Lessc::preg_quote(self::$commentMultiRight);
	 ...
	 *
	 * Consider the function Less_Lessc::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by Less_Lessc::match(), which behaves
	 ...
	 * avoid this, Less_Lessc::seek() is used to remember and set buffer positions.
	 *
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("Less_Lessc", "preg_quote"), $directives));
	...
	protected function expressionList(&$exps) {
		...
		$exps = Less_Lessc::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName = null) {
		...
		$value = Less_Lessc::compressList($values, ', ');
		return true;
	}
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("Less_Lessc", "preg_quote"), $stop);
	...
	protected function literal($what, $eatWhitespace = null) {
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = Less_Lessc::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.Less_Lessc::preg_quote($what), $m, !$until)) return false;
